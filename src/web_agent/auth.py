"""Browser Use Cloud auth for web-agent.

The model-facing contract stays small: cloud browser startup either has a key
or tells the agent to run `web-agent auth login`. OAuth details live here.
"""
...
# escape hatch so a dedicated web-agent client can be used once issued.
...
class CloudAuthRequired(RuntimeError):
    def __init__(self):
        super().__init__("cloud-auth-required: run `web-agent auth login`")
...
def client_id() -> str:
    return os.environ.get("WEB_AGENT_OAUTH_CLIENT_ID") or DEFAULT_CLIENT_ID
...
def start_browser_auth(*, open_url=True, timeout=AUTH_TIMEOUT_SECONDS) -> BrowserAuthStart:
    ...
    req = {
        ...
        "state": state,
        "device_name": os.environ.get("WA_DEVICE_NAME") or "web-agent",
    }
    try:
...
def start_device_auth(*, open_url=True) -> DeviceAuthStart:
    data = _post_json(
        ...
        {"client_id": client_id(), "device_name": os.environ.get("WA_DEVICE_NAME") or "web-agent"},
    )
...
def run_auth_cli(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(prog="web-agent auth")
