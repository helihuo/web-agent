...
def connect(name, timeout=1.0):
    ...
    get_session().record_event("ipc_connect", {"name": name})
    if not IS_WINDOWS:
...
def request(c, token, req):
    ...
    # 记录 IPC 请求（不记录敏感参数，仅记录方法名）
    method = req.get("method") or req.get("meta") or "unknown"
    get_session().record_event("ipc_request", {"method": method})
