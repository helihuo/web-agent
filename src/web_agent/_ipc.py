...
#   WA_RUNTIME_DIR — sock/port/pid. AF_UNIX sun_path is 104 bytes on macOS, so
#       the runtime dir must be short. Caller is responsible for keeping it
#       within budget. Falls back to WA_TMP_DIR (legacy single-dir callers),
#       then to the web-agent runtime dir.
#   WA_TMP_DIR — screenshots, debug overlays, daemon log. No path-length
...
# bare "wa" stems. Set WA_RUNTIME_DIR_SHARED=1 or WA_TMP_DIR_SHARED=1 when the
...
def _runtime_stem(name):  # "wa" when WA_RUNTIME_DIR isolates us, else "wa-<NAME>"
    _check(name)
    return "wa" if WA_RUNTIME_DIR and not WA_RUNTIME_DIR_SHARED else f"wa-{name}"


def _tmp_stem(name):  # "wa" when WA_TMP_DIR isolates us, else "wa-<NAME>"
    _check(name)
