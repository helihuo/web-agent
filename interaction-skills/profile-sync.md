...
## Python API (pre-imported in `web-agent`)
...
- Lower-level raw calls: `from web_agent.admin import _browser_use; _browser_use("/profiles/<id>", "DELETE")`. Pass the path *without* the `/api/v3` prefix — it's already on `BU_API`.
