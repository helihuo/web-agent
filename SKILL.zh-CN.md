...
- 保持连接模型简单：使用默认 daemon、`BU_NAME`、`BU_CDP_URL`、`BU_CDP_WS` 或 `start_remote_daemon(...)`。
...
- `BU_CDP_URL` 是 HTTP DevTools 端点；daemon 将其解析为 WebSocket。
- 在让云浏览器保持运行之前先询问；使用 `stop_remote_daemon(name)` 或 `PATCH /browsers/{id} {"action":"stop"}` 停止它们。
