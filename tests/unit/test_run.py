...
def test_cloud_bootstrap_on_headless_server(monkeypatch):
    ...
    with patch.object(sys, "argv", ["web-agent"]), \
         ...
         patch("web_agent.run.daemon_alive", return_value=False), \
         patch("web_agent.run._local_chrome_listening", return_value=False), \
         patch("web_agent.run.start_remote_daemon") as mock_start, \
         patch("web_agent.run.ensure_daemon"), \
         patch("web_agent.run.print_update_banner"):
        run.main()
...
def test_bad_stored_cloud_auth_does_not_bootstrap_or_crash(monkeypatch):
    ...
    with patch.object(sys, "argv", ["web-agent"]), \
         ...
         patch("web_agent.run.daemon_alive", return_value=False), \
         patch("web_agent.run._local_chrome_listening", return_value=False), \
         patch("web_agent.run.auth.get_browser_use_api_key", side_effect=run.auth.AuthError("auth file is not valid JSON")), \
         patch("web_agent.run.start_remote_daemon") as mock_start, \
         patch("web_agent.run.ensure_daemon"), \
         patch("web_agent.run.print_update_banner"):
        run.main()
...
def test_explicit_endpoint_does_not_break_daemon_alive_short_circuit(monkeypatch):
    ...
    with patch.object(sys, "argv", ["web-agent"]), \
         patch("sys.stdin", StringIO("x = 1")), \
         patch("web_agent.run.daemon_alive", return_value=True), \
         patch("web_agent.run._local_chrome_listening", return_value=False), \
         patch("web_agent.run.start_remote_daemon") as mock_start, \
         patch("web_agent.run.ensure_daemon"), \
         patch("web_agent.run.print_update_banner"):
...
def test_explicit_endpoint_does_not_break_local_chrome_short_circuit(monkeypatch):
    ...
    with patch.object(sys, "argv", ["web-agent"]), \
         patch("sys.stdin", StringIO("x = 1")), \
         patch("web_agent.run.daemon_alive", return_value=False), \
         patch("web_agent.run._local_chrome_listening", return_value=True), \
         patch("web_agent.run.start_remote_daemon") as mock_start, \
         patch("web_agent.run.ensure_daemon"), \
         patch("web_agent.run.print_update_banner"):
        run.main()
