...
- If that session is in a non-default Chromium browser, start it with a known remote-debugging port and pass `BU_CDP_URL` or `BU_CDP_WS` to Web Agent.
...
```bash
BU_NAME=qbo BU_CDP_URL=http://127.0.0.1:9223 web-agent -c 'print(page_info())'
```
