...
Web Agent patterns for `admin.shopify.com` and embedded Shopify apps.
...
Mike (or the human owner) must be logged into `admin.shopify.com` in the Chrome session that web-agent attaches to. The harness does NOT log in — it inherits the human's session.
