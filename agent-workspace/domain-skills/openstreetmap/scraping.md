...
**Do not use `http_get` without overriding `User-Agent`** — its default `Mozilla/5.0` is blocked by both APIs with HTTP 403. Pass `headers={"User-Agent": "web-agent/1.0"}` on every call.
...
**http_get works for GET requests** — pass `headers={"User-Agent": "web-agent/1.0"}`. For POST, use `urllib` directly (see example below).
...
```python
raw = http_get("https://overpass-api.de/api/status", headers={"User-Agent": "web-agent/1.0"})
print(raw)
...
def overpass_get_with_retry(query: str, max_retries: int = 3) -> dict:
    for attempt in range(max_retries):
        ...
        raw = http_get(url, headers={"User-Agent": "web-agent/1.0"})
        if raw.startswith("{"):
            return json.loads(raw)
...
**`http_get` default UA (`Mozilla/5.0`) is blocked by both APIs.** Always pass `headers={"User-Agent": "web-agent/1.0"}`. The `headers` kwarg in `http_get` does a `.update()` so it properly overrides the default. Confirmed: Mozilla/5.0 → 403 on Nominatim; `web-agent/1.0` → 200.

**Blocked User-Agent patterns on Nominatim**: `Mozilla/5.0`, `python-requests/*`, `Wget/*`. Accepted: any non-generic app-style UA like `web-agent/1.0`, `MyApp/2.0`, `curl/7.x`. Nominatim policy requires a descriptive UA with contact info, but in practice any non-library string passes.
